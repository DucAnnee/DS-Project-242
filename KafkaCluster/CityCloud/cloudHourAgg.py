...
SOURCE_TOPICS = ["district1.1h.sum", "district2.1h.sum"]
TARGET_TOPIC = "city.1h.sum"
# Use a unique group_id for your consumer group
CONSUMER_GROUP_ID = "city-hour-aggregator-group-py-v2"
EXPECTED_MESSAGES_PER_MINUTE_INTERVAL = 2  # 2 districts
...
def cloud_hour_worker():
    ...
    try:
        ...
        while running:
            try:
                for message in consumer:
                    ...
                    try:
                        msg_timestamp_str = data["timestamp_hour"]
                        consumption_value = data["hourly_total_consumption"]
                        ...
                        if current_msg_minute_key_str not in hour_aggregates:
                            hour_aggregates[current_msg_minute_key_str] = {
                                "hourly_total_consumption": 0.0,
                                "message_count": 0,
                            }

                        # Aggregate consumption and increment count
                        hour_aggregates[current_msg_minute_key_str][
                            "hourly_total_consumption"
                        ] += float(consumption_value)
                        hour_aggregates[current_msg_minute_key_str][
                            "message_count"
                        ] += 1

                        current_agg_data = hour_aggregates[current_msg_minute_key_str]
                        logger.debug(
                            f"Aggregated for {current_msg_minute_key_str}: "
                            f"Sum={current_agg_data['hourly_total_consumption']:.2f}, "
                        ...
                        if (
                            ...
                            output_payload = {
                                "timestamp_hour": current_msg_minute_key_str,
                                "hourly_total_consumption": round(
                                    current_agg_data["hourly_total_consumption"], 2
                                ),
                            }
                            producer.send(
        ...
        if producer and hour_aggregates:
            logger.info(
                f"Flushing {len(hour_aggregates)} remaining aggregate(s) during shutdown..."
            )
            sorted_minute_keys = sorted(hour_aggregates.keys())
            for minute_key_str in sorted_minute_keys:
                agg_data = hour_aggregates[minute_key_str]
                output_payload = {
                    "timestamp_hour": minute_key_str,
                    "hourly_total_consumption": round(
                        agg_data["hourly_total_consumption"], 2
                    ),
                }
