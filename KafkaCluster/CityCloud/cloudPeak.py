...
SOURCE_TOPICS = ["district1.1h.peak", "district2.1h.peak"]
TARGET_TOPIC = "city.1h.peak"  # Output: hour-level peak from a single household
# Use a unique group_id for your consumer group
CONSUMER_GROUP_ID = "city-hourly-household-peak-detector-group-py-v2"

# Assuming 3 wards, 5 houses/ward, 1 message/minute/house:
# 3 wards * 5 houses/ward * 60 minutes/hour = 900 messages per hour
EXPECTED_MESSAGES_PER_HOUR_INTERVAL = 2
        ...
        while running:
            try:
                for message in consumer:  # This loop blocks until a message or timeout
                    ...
                    try:
                        raw_meter_id = data["peak_meter_id"]
                        raw_timestamp_str = data[
                            "peak_timestamp"
                        ]  # This is the exact timestamp from the meter
                        raw_consumption = data["peak_household_consumption"]
